...
class Injector {
	const CONSTANT   = 'Injector::CONSTANT';
	const GLOBAL     = 'Injector::GLOBAL';
	const INSTANCE   = 'Injector::INSTANCE';
	const CHAIN_CALL = 'Injector::CHAIN_CALL';
	/**
	...
	public function addRule( string $name, array $rule ): self {
		$injector = clone $this;
		$this->addRuleTo( $injector, $name, $rule );

		return $injector;
	}

	/**
	 * Add rules as array. Useful for JSON loading $injector->addRules(json_decode(file_get_contents('foo.json'));
	 *
	...
	private function addRuleTo( Injector $injector, string $name, array $rule ) {
		if ( isset( $rule['instanceOf'] ) && ( ! array_key_exists( 'inherit', $rule ) || $rule['inherit'] === true ) ) {
			$rule = array_replace_recursive( $injector->getRule( $rule['instanceOf'] ), $rule );
		}
		...
		unset( $injector->instances[ $name ], $injector->cache[ $name ] );
		$injector->rules[ ltrim( strtolower( $name ), '\\' ) ] = array_replace_recursive( $injector->getRule( $name ), $rule );
	}
	...
	/**
	 * Looks for Injector::INSTANCE, Injector::GLOBAL or Injector::CONSTANT array keys in $param and when found returns
	 * an object based on the value see {@link https:// r.je/dice.html#example3-1}
	 *
	...
	private function expand( $param, array $share = [], bool $createFromString = false ) {
		if ( is_array( $param ) ) {
			//if a rule specifies Injector::INSTANCE, look up the relevant instance
			if ( isset( $param[ self::INSTANCE ] ) ) {
				...
				//Support Injector::INSTANCE by creating/fetching the specified instance
	...
	private function getParams( \ReflectionMethod $method, array $rule ) {
		...
		return function ( array $args, array $share = [] ) use ( $paramInfo, $rule ) {
			// Now merge all the possible parameters: user-defined in the rule via constructParams, shared instances and the $args argument from $injector->create();
